...
export default function EmailOTPPage() {
  ...
  }, [hasAutoSubmitted, otp, email, isLoading]);

  // Auto-submit when user enters 6th digit
  useEffect(() => {
    const now = Date.now();
    // Prevent auto-submit if we just submitted within the last 2 seconds
    if (otp.length === 6 && email && !isLoading && now - lastSubmitTime > 2000) {
      // Auto-submit the form
      const submitForm = async () => {
        await handleVerifyOTP(new Event('submit') as any);
      };
  ...
  }, [otp, email, isLoading, lastSubmitTime]);
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        // Always redirect to profile for regular login (with validation)
  ...
  const handleSendNewCode = async () => {
    ...
    if (resendCooldown > 0) {
      return; // Don't send if still in cooldown
    }

    setIsLoading(true);
    setLastResendTime(Date.now()); // Reset the timer
    try {
          ...
          <form onSubmit={handleVerifyOTP} className="space-y-4">
            ...
            {!email && (
              <div className="space-y-2">
                ...
                <Input id="email" type="email" placeholder="Enter your email" autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} required />
              </div>
            )}
            ...
            <div className="flex justify-center text-sm">
              <Button type="button" variant="link" onClick={handleSendNewCode} disabled={isLoading || !email || resendCooldown > 0} className="p-0 h-auto">
